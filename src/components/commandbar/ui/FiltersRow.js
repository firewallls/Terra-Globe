export default function FiltersRow(container, { regions, popFilters, activeRegion, activePop, onFilterRegion, onFilterPop }) {
  container.innerHTML = `
    <div class="flex items-center gap-1.5 px-4 py-2.5 border-b border-border shrink-0 overflow-x-auto no-scrollbar whitespace-nowrap">
      ...
      ${popFilters.map(p => `
        <div 
          class="cmd-filter-chip px-2.5 h-[26px] flex items-center justify-center bg-surface-mid border ${activePop === p.value ? 'border-signal text-signal bg-signal-tint' : 'border-border text-text-secondary'} rounded-[13px] font-familjen text-[10px] font-medium tracking-widest cursor-pointer uppercase transition-all select-none hover:border-border-strong hover:text-text-primary shrink-0"
          data-pop="${p.value}"
        >${p.label}</div>
      `).join('')}
    </div>
