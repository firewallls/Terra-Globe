  ...
  container.innerHTML = `
    <div class="flex items-center px-4 h-[38px] border-b border-border shrink-0 gap-0 overflow-x-auto no-scrollbar whitespace-nowrap">
      ...
      <div class="flex items-center">
        ${sortOptions.map(s => `
          <div 
            class="px-2.5 h-[38px] flex items-center font-familjen text-[10px] font-bold tracking-widest uppercase cursor-pointer relative transition-colors select-none shrink-0 ${currentSort === s.value ? 'text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}"
            data-sort="${s.value}"
          >
            ${s.label}
