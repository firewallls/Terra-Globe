(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(e){let t=e,n=t.getContext(`2d`),r=[],i=()=>{t.width=window.innerWidth,t.height=window.innerHeight,a()};function a(){r=[],[{count:50,radius:2,opacity:[.7,.9],speed:.2,color:`#EEF2FF`},{count:75,radius:1.5,opacity:[.4,.6],speed:.15,color:`#EEF2FF`},{count:150,radius:1,opacity:[.2,.4],speed:.1,color:`#EEF2FF`},{count:6,radius:2.5,opacity:[.5,.5],speed:0,color:`#00E5FF`}].forEach(e=>{for(let n=0;n<e.count;n++)r.push({x:Math.random()*t.width,y:Math.random()*t.height,dx:(Math.random()-.5)*e.speed,dy:(Math.random()-.5)*e.speed,radius:e.radius,opacity:e.opacity[0]+Math.random()*(e.opacity[1]-e.opacity[0]),color:e.color})})}function o(){n.fillStyle=`#000000`,n.fillRect(0,0,t.width,t.height),r.forEach(e=>{n.save(),n.beginPath(),n.globalAlpha=e.opacity,n.fillStyle=e.color,e.color===`#00E5FF`&&(n.shadowBlur=10,n.shadowColor=`#00E5FF`),n.arc(e.x,e.y,e.radius,0,Math.PI*2),n.fill(),n.restore()})}function s(){r.forEach(e=>{e.x+=e.dx,e.y+=e.dy,e.x<0&&(e.x=t.width),e.x>t.width&&(e.x=0),e.y<0&&(e.y=t.height),e.y>t.height&&(e.y=0)}),t.style.position=`fixed`,t.style.zIndex=`-1`,o(),requestAnimationFrame(s)}window.addEventListener(`resize`,i),i(),s()}function t(e){e.innerHTML=`
  ...
  `}var n=e=>e?e>=1e9?(e/1e9).toFixed(1)+`B`:e>=1e6?(e/1e6).toFixed(1)+`M`:e>=1e3?(e/1e3).toFixed(0)+`K`:e.toString():`—`,r=async()=>(await fetch(`https://restcountries.com/v3.1/all?fields=name,capital,region,subregion,population,area,cca3,flag`)).json(),i=(e,t,n)=>{let r=t.search.trim().toLowerCase();return e.filter(e=>{if(t.region&&e.region!==t.region||t.pop&&(e.population||0)<t.pop)return!1;if(!r)return!0;let n=(e.name?.common||``).toLowerCase(),i=(e.capital?.[0]||``).toLowerCase(),a=(e.region||``).toLowerCase();return n.includes(r)||i.includes(r)||a.includes(r)}).sort((e,t)=>{let r={"pop-desc":()=>(t.population||0)-(e.population||0),"pop-asc":()=>(e.population||0)-(t.population||0),"area-desc":()=>(t.area||0)-(e.area||0),default:()=>(e.name?.common||``).localeCompare(t.name?.common||``)};return(r[n]||r.default)()})};function a(e,{onInput:t,onClose:n}){e.children.length===0&&(e.innerHTML=`
    ...
    <div class="flex items-center gap-1.5 px-4 py-2.5 border-b border-border shrink-0 overflow-x-auto no-scrollbar whitespace-nowrap">
      <div 
        class="cmd-filter-chip px-2.5 h-[26px] flex items-center justify-center bg-surface-mid border ${r?`border-border text-text-secondary`:`border-signal text-signal bg-signal-tint`} rounded-[13px] font-familjen text-[10px] font-medium tracking-widest cursor-pointer uppercase transition-all select-none hover:border-border-strong hover:text-text-primary shrink-0"
        data-region=""
      >ALL</div>
      ${t.map(e=>`
        <div 
          class="cmd-filter-chip px-2.5 h-[26px] flex items-center justify-center bg-surface-mid border ${r===e?`border-signal text-signal bg-signal-tint`:`border-border text-text-secondary`} rounded-[13px] font-familjen text-[10px] font-medium tracking-widest cursor-pointer uppercase transition-all select-none hover:border-border-strong hover:text-text-primary shrink-0"
          data-region="${e}"
        >${e}</div>
      `).join(``)}
      ${n.map(e=>`
        <div 
          class="cmd-filter-chip px-2.5 h-[26px] flex items-center justify-center bg-surface-mid border ${i===e.value?`border-signal text-signal bg-signal-tint`:`border-border text-text-secondary`} rounded-[13px] font-familjen text-[10px] font-medium tracking-widest cursor-pointer uppercase transition-all select-none hover:border-border-strong hover:text-text-primary shrink-0"
          data-pop="${e.value}"
        >${e.label}</div>
      `).join(``)}
    </div>
  `,e.querySelectorAll(`[data-region]`).forEach(e=>{e.addEventListener(`click`,()=>a(e.dataset.region))}),e.querySelectorAll(`[data-pop]`).forEach(e=>{e.addEventListener(`click`,()=>o(parseInt(e.dataset.pop,10)))})}function s(e,{sortOptions:t,currentSort:n,onSort:r}){e.innerHTML=`
    <div class="flex items-center px-4 h-[38px] border-b border-border shrink-0 gap-0 overflow-x-auto no-scrollbar whitespace-nowrap">
      <div class="font-azeret-mono text-[9px] font-light tracking-[0.15em] text-text-tertiary uppercase mr-3 shrink-0">Sort:</div>
      <div class="flex items-center">
        ${t.map(e=>`
          <div 
            class="px-2.5 h-[38px] flex items-center font-familjen text-[10px] font-bold tracking-widest uppercase cursor-pointer relative transition-colors select-none shrink-0 ${n===e.value?`text-text-primary`:`text-text-tertiary hover:text-text-secondary`}"
            data-sort="${e.value}"
          >
            ${e.label}
  ...
  `}function l(e,{countries:t,isLoading:r,searchQuery:i}){if(r){e.innerHTML=`<div class="p-8 text-center font-azeret-mono text-[11px] tracking-widest text-text-tertiary">Loading countries…</div>`;return}let a=e.querySelector(`#cmd-results-list`),o=a?a.scrollTop:0;if(a||=(e.innerHTML=`<div id="cmd-results-list" class="overflow-y-auto flex-1 h-full scrollbar-thin scrollbar-thumb-signal-tint"></div>`,e.querySelector(`#cmd-results-list`)),t.length===0){a.innerHTML=`<div class="p-8 text-center font-azeret-mono text-[11px] tracking-widest text-text-tertiary">No results for "<em>${i}</em>"</div>`;return}a.innerHTML=t.map(e=>`
  ...
  `}function E(e){e.style.zIndex=`1`,e.innerHTML=`
    <div class="fixed top-0 left-0 right-0 h-[5.34vh] min-h-[48px] grid grid-cols-[1fr_auto_1fr] items-stretch px-4 sm:px-6 bg-overlay-topbar backdrop-blur-topbar shadow-topbar text-text-primary font-sans z-[200]">
